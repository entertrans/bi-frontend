...
const SiswaAlumniTable = () => {
  ...
  const filteredData = dataSiswa.filter((siswa) => {
    const kelasMatch = selectedKelas
      ...
      : true;
    const taMatch = selectedTahunAjaran
      ? siswa.ta_lulus === selectedTahunAjaran
      : true;
    const searchMatch =
      siswa.siswa_nama.toLowerCase().includes(searchTerm.toLowerCase()) ||
      siswa.siswa_nis.includes(searchTerm);
    return kelasMatch && taMatch && searchMatch;
  });
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {tahunAjaranOptions.map((ta) => (
              <option key={ta.label} value={ta.label}>
                {ta.label}
              </option>
            ))}
          </select>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-100 dark:bg-gray-700">
            <tr>
              <th className="px-6 py-3 text-left w-40">Nama</th>
              ...
              <th className="px-6 w-32 py-3 text-left">T/A</th>
            ...
            {paginatedData.map((siswa) => {
              return (
                <tr key={siswa.siswa_id}>
                  ...
                  <td className="px-6 py-4">{siswa.ta_lulus}</td>
...
export default SiswaAlumniTable;
