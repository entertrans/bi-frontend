...
import {
  ...
  deleteTestSoal,
  updateTestSoal,
  createTestSoal, // Tambah import untuk fungsi createTestSoal
...
const SlideTambahSoal = ({ isOpen, onClose, test }) => {
  ...
  // ✅ Fungsi baru untuk menangani penambahan atau update soal
  const handleSaveSoal = async (soalData) => {
    try {
      let response;
      if (soalData.testsoal_id) {
        // Jika ada testsoal_id, lakukan update
        response = await updateTestSoal(soalData.testsoal_id, soalData);
      } else {
        // Jika tidak ada, lakukan create
        response = await createTestSoal({
          ...soalData,
      ...
      if (response.success) {
        ...
        showAlert("Gagal menyimpan soal.", "error");
      }
    } catch (error) {
